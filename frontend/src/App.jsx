      ...
      <div className="layout-container">
        ...
        <aside className="sidebar">
          ...
          <ul className="sidebar-menu">
            ...
            <li 
              className={`sidebar-item ${activeTab === 'admin' && adminTab === 'products' ? 'active' : ''}`}
              onClick={() => { setActiveTab('admin'); setAdminTab('products'); }}
            >
              <Package size={20} />
              <span>Products</span>
            </li>
            <li 
              className={`sidebar-item ${activeTab === 'admin' && adminTab === 'categories' ? 'active' : ''}`}
              onClick={() => { setActiveTab('admin'); setAdminTab('categories'); }}
            >
              <Grid size={20} />
              <span>Categories</span>
            </li>
            <li 
              className={`sidebar-item ${activeTab === 'admin' && adminTab === 'payment_methods' ? 'active' : ''}`}
              onClick={() => { setActiveTab('admin'); setAdminTab('payment_methods'); }}
            >
              <CreditCard size={20} />
              <span>Payment Methods</span>
            </li>
            <li 
              className={`sidebar-item ${activeTab === 'admin' && adminTab === 'tables' ? 'active' : ''}`}
            ...
            <li 
              ...
              onClick={() => { setActiveTab('admin'); setAdminTab('coupons'); }}
            >
              <Tag size={20} />
              <span>Coupons & Promotions</span>
            </li>
            <li 
              className={`sidebar-item ${activeTab === 'admin' && adminTab === 'employees' ? 'active' : ''}`}
              onClick={() => { setActiveTab('admin'); setAdminTab('employees'); }}
            >
              <Users size={20} />
              <span>Employees</span>
            </li>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
              <div>
                <h1 style={{ fontSize: '2rem', marginBottom: '6px' }}>
                  ...
                  {adminTab === 'payment_methods' && 'Payment Methods'}
                  {adminTab === 'tables' && 'Tables & Floors'}
                  {adminTab === 'coupons' && 'Coupons & Promotions'}
                  {adminTab === 'employees' && 'Employees'}
                </h1>
                <p style={{ color: 'var(--text-secondary)' }}>Manage your GatherPoint configurations</p>
              </div>
            </div>

            {/* ADMIN SCREEN 0: DASHBOARD */}
            {adminTab === 'dashboard' && (
              <div className="glass-panel" style={{ padding: '40px', textAlign: 'center' }}>
                <h2>Welcome to your Dashboard</h2>
                <p style={{ color: 'var(--text-secondary)', marginTop: '10px' }}>Select an option from the sidebar to manage your cafe's operations.</p>
              </div>
            )}

            {/* ADMIN SCREEN: PAYMENT METHODS PLACEHOLDER */}
            {adminTab === 'payment_methods' && (
              <div className="glass-panel" style={{ padding: '40px', textAlign: 'center' }}>
                <h2>Payment Methods</h2>
                <p style={{ color: 'var(--text-secondary)', marginTop: '10px' }}>Configure your payment gateways and methods here.</p>
              </div>
            )}

            {/* ADMIN SCREEN: TABLES & FLOORS PLACEHOLDER */}
            {adminTab === 'tables' && (
              <div className="glass-panel" style={{ padding: '40px', textAlign: 'center' }}>
                <h2>Tables & Floors</h2>
                <p style={{ color: 'var(--text-secondary)', marginTop: '10px' }}>Manage floor plans and seating arrangements here.</p>
              </div>
            )}
